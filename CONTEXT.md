...
- **Vision**: Single binary (`pylon`) that provides a unified interface to all deployed services in the jredh-dev infrastructure.
- **Status**: Initial development - v0.1.0

## Architecture Decisions
- **Tech Stack**: Go, stdlib only (no external CLI framework), consistent with ctl's approach
- **Design Patterns**: Service-namespaced subcommands (`pylon cal feed create`, `pylon cal event add`), HTTP client per service
...
- **Next Steps**: Merge PR, deploy cal service, test pylon against live API, add more services as they come online

## Task Tracking
### Completed
- [x] Project scaffolding (go.mod, LICENSE, repo setup on GitHub + Gitea)
- [x] Config package (PYLON_CAL_URL env var)
...
- [x] Renamed from xor to pylon (repo, module path, binary, env vars, all references)
...
## Development Notes
- **Build**: `make build` (binary at `bin/pylon`)
- **Install**: `make install` (copies to GOPATH/bin)
- **Test**: `make test` or `go test ./...`
- **Config**: `export PYLON_CAL_URL=http://localhost:8085` (default)
- **Release**: Push a `v*` tag to trigger GitHub release workflow

## Session Log
- **[2026-02-18]**: Created xor repo, wrote cal client + CLI + config + tests. Renamed to pylon. Matching ctl patterns for versioning, Makefile, release workflow, CHANGELOG format.
